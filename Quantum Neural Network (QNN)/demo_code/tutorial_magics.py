...
class Copyright(Magics):
    ...
    def qiskit_copyright(self, line="", cell=None):
        ...
        html += (
            "<p>Any modifications or derivative works of this code must retain this<br>"
        )
        html += (
            "copyright notice, and modified files need to carry a notice indicating<br>"
        )
...
class VersionTable(Magics):
    ...
    def qiskit_version_table(self, line="", cell=None):
        ...
        qiskit_modules = {
            module.split(".")[0] for module in modules.keys() if "qiskit" in module
        }

        for qiskit_module in qiskit_modules:
            packages[qiskit_module] = getattr(
                modules[qiskit_module], "__version__", None
            )
